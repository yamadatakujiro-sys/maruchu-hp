...
### 現在の作業
**ココナラでスタートキットを「新規出品」中**（オーナーが画面で入力している）。
貼る文はすべて `docs/COCONALA-LISTING-B.md` にある。**入力順の表**と**各欄の文字数上限の表**もそこにある。
- 提供形式＝**「テキストチャット・データ納品」**（＝昔の「通常サービス」。画面表記が変わっている）
- サービス内容欄は**1,000字以内** → 貼るのは **③の短縮版（810字）**。③-2の全文版は他販路用

### すぐやる残タスク（優先順）
1. ⚠️ **旧出品 `4289713` を非公開にする**（「月額¥9,800・運用代行」で公開中＝誤販売リスク）
2. 新規出品を公開 → **URLをこのファイルに追記**
3. **使い方動画（約5分）を撮る**（台本＝`ONBOARDING-VIDEO-SCRIPT.md`）→ URLを
   `starter-kit/はじめにお読みください.md` と出品文に追記
4. `~/ai-office-starter-template` を最新のstarter-kitと同期（ハッシュタグ5個修正・日本語ルール）→ スマホで再テスト
5. ヤフオクにライト版¥5,000をテスト出品（文＝`YAHOO-LISTING-LIGHT.md`／**出品前に本人が規約確認**）
6. 特商法表示（`LEGAL-QUESTIONS.md §5`）／2業種目のテンプレ
