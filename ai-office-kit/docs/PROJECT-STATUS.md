...
✅**トンネル切れの恒久対策 実装（2026-07-17・キット側完了／Mac側適用待ち）**：Mac再起動でcloudflaredトンネルが落ちると無反応になる既知の弱点に対し、launchd自動復帰を実装。`office.conf` の `TUNNEL_CMD` に名前付きトンネルの起動コマンドを設定→`install.sh` で `com.lineaioffice.tunnel` が keepalive+RunAtLoad 常駐（`bin/tunnel-run.sh`）。`watchdog.sh` にトンネル死活チェックも追加。手順・検証は `RUNBOOK.md` §D。**残：Mac実機で①名前付きトンネル確認（quick tunnel なら要作成）②`TUNNEL_CMD`設定③`install.sh`実行④E2E検証（LINE 1通→ターミナル操作なしで自動応答）。**
次にやること：
