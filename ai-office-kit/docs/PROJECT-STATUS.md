...
⚠️**既知の弱点＝トンネル切れ（要恒久対策）**：Mac再起動でcloudflaredトンネルが落ちると、bridgeは生きててもLINE着信が届かず無反応になる。当面は「LINEで依頼→ターミナルでリーダー手動起動」で回避（手順・注意は`RUNBOOK.md` §D）。顧客Macでも起きるため、トンネルのlaunchd自動復帰が宿題。
次にやること：
