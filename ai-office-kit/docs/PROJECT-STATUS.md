...
- **LINEステップ配信シナリオ**：`docs/LINE-STEP-SCENARIO.md` に設計の考え方（リストマーケ4ステップ／教育・セールスの2方向／各通=目的・CTA・ユーザー心理／初回オファー先出し／CTAは直球）＋配信文6通（ストーリー版）を保存。
...
次にやること：
- **公式LINE設定**：あいさつメッセージ設定＋ステップ配信(1〜5)登録＋友だち追加URLをXのbio/固定に貼る（`docs/LINE-STEP-SCENARIO.md`）。応答=チャット(手動)。
