...
次にやること：
- **デモ動画**を作成→「デモ」応答＆あいさつ画像リンクを動画に差し替え（今は「準備中」表記）。
- サービス紹介ページ（note/LP）ができたらリッチメニューCを外部リンクにしてもよい。
- **Stripe**（準備中・手順は `docs/STRIPE-SETUP.md`）：登録→初期¥39,800(一括)＋月¥9,800(サブスク)の商品作成→Payment Link→特商法表記・プライバシーポリシー。Payment Links(コード不要)方式。相談成約後にLINEでリンク案内。
