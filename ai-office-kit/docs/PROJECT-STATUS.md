...
### 現在の作業
✅**ココナラの新規出品が完了（2026-08-12）**＝スタートキットを売り切り3価格（基本¥5,000＋有料オプション）で出品。
提供形式＝**「テキストチャット・データ納品」**。貼った文は `docs/COCONALA-LISTING-B.md`（③の810字短縮版）。
- ⚠️**新出品のURLが未記録** → オーナーからURLをもらって**このファイルと`COCONALA-LISTING-B.md`に追記する**。

### すぐやる残タスク（優先順）
1. 🚨 **旧出品 `4289713` を非公開にする**（「月額¥9,800・運用代行／お客様はPC不要」で公開中＝**現行商品と矛盾＝誤販売リスク**）。
   売上0なので削除でも可だが、非公開なら取り消せるので**まず非公開**。
2. **新出品のURLを記録**（このファイル §いまここ ＋ `COCONALA-LISTING-B.md` 冒頭）。
   併せて**買い手目線でライブ確認**（3価格が有料オプションで正しく出るか／画像5枚が新素材か／
   「Claude有料契約が必要」「LINEは使いません」が本文に出ているか）。
3. **使い方動画（約5分）を撮る**（台本＝`ONBOARDING-VIDEO-SCRIPT.md`・そのまま読める）→ YouTube限定公開 →
   URLを `starter-kit/はじめにお読みください.md`／出品文の「含まれるもの」／納品案内文に反映。
   ＝**出品文で「動画つき」と約束済みなので、売れる前に必須**。
4. `~/ai-office-starter-template` を最新の `starter-kit/` と同期（**ハッシュタグ5個修正・日本語必須ルール**が未反映）
   → スマホで再テスト。**未同期のまま納品すると規定超えのハッシュタグを出す＝商品の欠陥**。
5. **特商法表示**（`LEGAL-QUESTIONS.md §5`）。ココナラ内は運営表示で足りるが、**ヤフオク・直販・Stripeでは自分の表示が必要**。
6. ヤフオクにライト版¥5,000をテスト出品（文＝`YAHOO-LISTING-LIGHT.md`／**出品前に本人が規約確認**）。
7. 集客＝**実績0→1は「広く撒く」より直接オファー**（`SALES-MONITOR-OUTREACH.md`）。X/Instagramに出品告知。
8. 2業種目の業種テンプレ（1業種目＝整備工場・鈑金塗装は完成）。
9. LINE版（¥49,000〜）を出す場合は**新規で別出品**＋旧素材 `assets/coconala-thumbnail/` の月額¥9,800記載を修正。
