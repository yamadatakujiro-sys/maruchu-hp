...
2. `~/ai-office-starter-template` を最新の `starter-kit/` と同期（**ハッシュタグ5個修正・日本語必須ルール**が未反映）
   → スマホで再テスト。**未同期のまま納品すると規定超えのハッシュタグを出す＝商品の欠陥**。
   コマンド＝`docs/STARTER-KIT-DELIVERY.md` の「テンプレ同期」節。
