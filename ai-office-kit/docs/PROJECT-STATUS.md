...
### すぐやる残タスク（優先順）
1. **使い方動画（約5分）を作る**＝**映像は無音の画面収録／声はAI音声／字幕は自動**
   （オーナー要望「声がガサガサなので読み上げも全部AIに任せたい」／2026-08-12）。
   ⚠️**全部AI生成（アバター）にはしない**＝本物のアプリが動いていることが価値なので信用が落ちる。
   - 貼る原稿＝**`docs/ONBOARDING-VIDEO-TTS.md`**（記号なし・読み方修正済み・約1,100字・⓪〜⑤の6ブロック）
   - 本命ツール＝**Vrew**（AI音声＋自動字幕＋編集が1つで完結）。⚠️**商用はLightプラン（月1,290円程度）から。
     無料プランは商用不可**＝売り物には使えない。無料案＝VOICEVOX（キャラ声寄り）／音読さん（商用はクレジット表記必須）。
   - → YouTube**限定公開** → URLを `starter-kit/はじめにお読みください.md`／`COCONALA-LISTING-B.md`／納品案内文の**3か所**に反映。
   ＝**出品文で「動画つき」と約束済みなので、売れる前に必須**。
