...
### ★スタートキット 実機検証：スマホだけで動作を確認（2026-08-11）
**納品形態が確定＝「顧客の非公開GitHubリポジトリ」**。オーナーのiPhone（Claudeアプリ）で通しテストに成功。
- **入り方**：Claudeアプリ →「コード」→ 新規セッション画面の**リポジトリ選択チップ**（GitHubアイコン付き）をタップ→リポジトリを選ぶ→ブランチ`main`のまま送信。
- **検証リポジトリ**：`yamadatakujiro-sys/ai-office-starter-template`（private・**キットをルート直下**に配置）。
  ⚠️ Claude Code(cloud)は**リポジトリ直下の`CLAUDE.md`と`.claude/agents/`を読む**ため、奥のフォルダに入れてはいけない。
  ⚠️ 本セッションのGitHub連携はスコープ外のため**リポジトリ作成/読取は不可**（オーナーがWebで作成→Macから`git push`で投入）。
- **動作結果（すべて期待どおり）**：①リーダーとして応答 ②「SNS担当に渡します」と**振り分けを明示** ③`納品/`に保存する旨を提示
  ④**`会社情報.md`が空欄なのを検知し、3つだけヒアリング**（＝設計した「聞きすぎない」ルールが機能）
  ⑤「会社情報.mdに書けば毎回聞かずに済む」と自ら提案。
- **見つかった不具合→修正済み**：初手が英語（`I'll start by reading...`）だった。顧客向け商品として不可のため
  `CLAUDE.md`に「話し方（最優先）」を追加＋**全8エージェントの共通ルールに「必ず日本語で応答」**を追記。
  併せて**英語のID名（`sns`/`designer`）を顧客への説明に出さない**（日本語の職種名で話す）ルールも追加。
- **結論**：**スマホだけで完結する売り切り商品として成立**。PC不要＝店舗系にも売れる＝入口商品として強い。
  → テンプレを修正したら、顧客ごとに**コピーして`会社情報.md`を埋めるだけ**で納品可能。
- 残：顧客向け納品手順書（GitHubアカウント作成〜アプリでの開き方）／業種別テンプレ／`ai-office-starter-template`への修正反映。
