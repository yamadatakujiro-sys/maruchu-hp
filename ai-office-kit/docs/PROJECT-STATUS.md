...
✅**「着手の一報」強化（2026-07-02）**：担当が作業を先に始めて着手一報を省略する事象へ対策。`spawn-watcher.mjs`/`SESSION-MODE-TEMPLATE.md`で「最初の必須ツール呼び出し・省略厳禁」に強化（チーム連携の見える化＝信頼性）。反映は各Macで`install.sh`再実行。
次にやること：
- **デモ動画**を作成→「デモ」応答＆あいさつ画像リンクを動画に差し替え（今は「準備中」表記）。台本・手順は`DEMO-VIDEO.md`。
