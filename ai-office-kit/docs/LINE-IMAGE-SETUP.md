...
## テキスト成果物は「本文そのもの」をLINEへ（画像/動画と同じ思想）
キャプション・投稿文・記事などの**文章成果物は、ファイルパスだけで終えず本文そのものをLINEに送る**
（共通層 `templates/SESSION-MODE-TEMPLATE.md` の完成報告ルールに明記）。画像・動画は R2 経由で"実物"を、
テキストは `send_message` で"本文"を届ける＝**どの成果物もLINEを見るだけで完結**（顧客がファイル場所を知らなくてよい）。
長文は約4,500文字ごとに分割して送る。
