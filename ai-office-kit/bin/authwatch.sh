...
if printf '%s' "$OUT" | grep -Eiq "$AUTH_ERROR_RE"; then
  ...
  if [ $(( NOW_TS - last )) -ge "$COOLDOWN_SEC" ]; then
    echo "$NOW_TS" > "$STAMP"
    if [ -n "${OWNER_FRIEND_ID:-}" ]; then
      "$NODE_BIN" "$NOTIFY_BIN" --to "$OWNER_FRIEND_ID" \
        "🔔【定期点検】AI社員のログイン(認証)が切れています。Macで Claude Code を開き /login で再ログインしてください。今のうちに直せば無反応を防げます。" \
        >/dev/null 2>&1 || true
    fi
  fi
else
