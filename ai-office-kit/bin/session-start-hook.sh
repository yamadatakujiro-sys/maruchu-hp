#!/bin/bash
# =============================================================
#  LINE AIオフィス — session-start-hook（フック①）
#
#  SessionStart 時に「起動ルーチン（5分Cron監視）」を additionalContext で注入する。
#  使い方: session-start-hook.sh <lineAccountId>   （例: member-designer）
#  ※ ~/.claude/settings.json の hooks.SessionStart に登録して発火する。
#
...
LINEで届いた依頼にのみ対応します（定期チェックはしません）。CronCreate は実行しないこと。
1. mcp__${MCP_NAME}__get_ai_conversations (lineAccountId: ${LEADER_ID}, limit: 20) でオーナーとの会話を確認し、未返信があれば対応。
2. 作業依頼は各担当の ${OFFICE_HOME}/members/{担当dir}/inbox/task.md に振り分け（依頼元／目的／やること／成果物パス／完了条件）。担当は自動起動する。
